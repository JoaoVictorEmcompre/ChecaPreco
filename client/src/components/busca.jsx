...
export default function CampoDeBusca({ value, onChange, onSubmit }) {
  ...
  // >>> VERSÃO CORRIGIDA <<<
  // Este useEffect agora lida corretamente com a troca de câmera.
  useEffect(() => {
    // A condição !controlsRef.current foi REMOVIDA
    if (selectedDeviceId && openScanner && videoDevices.length > 0) {
      ...
      // A função startScanner já contém a lógica para parar o scanner anterior
      // antes de iniciar um novo, então podemos chamá-la diretamente.
